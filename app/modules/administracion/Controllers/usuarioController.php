...
class Administracion_usuarioController extends Administracion_mainController
{
	...
	public function indexAction()
	{
		...
		// $this->_view->list_modulos = $this->getModulos();
		// $this->_view->list_modulos_sub = $this->getModulosSub();
		// $this->_view->permisos = $this->getPermisos();
	}
	...
	public function permisosAction(){
		...
		$this->mainModel->delete_permisos($user_id);

		$modulos = $_POST["modulos_id"];

        $data = array();
        $data["user_mod_user"] = $user_id;
        $data["user_mod_mod"] = print_r($modulos,true);
		
        foreach ($modulos as $valores) {
            $data2 = array();
            $data2["user_mod_user"] = $user_id;
            $data2["user_mod_mod"] = $valores;
            $this->mainModel->insert_permisos($data2);
        }
        
        //LOG
        $data['log_log'] = print_r($data,true);
        $data['log_tipo'] = "CREAR PERMISOS";
        $logModel = new Administracion_Model_DbTable_Log();
        $logModel->insert($data);
    ...
    protected function getFilter()
    {
        ...
        if (Session::getInstance()->get($this->namefilter)!="") {
			...
			$filtros = $filtros." AND (user_level != '16')";
		}
        return $filtros;
    }
